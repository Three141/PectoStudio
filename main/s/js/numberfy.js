(function($) { $.fn.numberfy = function() {
	...
	$(this).filter('textarea').each(function() {
		...
		var container;
		$(this).appendTo(container = $('<div>').addClass('lineNumberfier').css({position: 'relative', height: 'calc(100% - 300px)'}).insertBefore($(this))); //$(this)[0].offsetHeight
		...
		//set up the line numbers col and position it to right of textarea
		var col = $('<div>').css({background: column_BG, color: column_text, width: 25, position: 'absolute', overflow: 'hidden', right: 0, top: 0, height: '100%'}).addClass('lineNumsCol').prependTo($(this).parent());
		...
		measurer.css(lineAffectingStyles).css({width: $(this).width(), paddingLeft: $(this).css('padding-right'), paddingRight: $(this).css('padding-right')});
		...
		//pad container right by width of line numbers col, so no overlap between col and textarea
		container.css('padding-right', col[0].offsetWidth);

		//on load, and on keyup in this textarea, set lines
		doLines($(this));
		//$(this).keydown(function() { doLines($(this)); });
        var obj = $(this);
        setInterval(function() { doLines(obj); }, 100);
        $(this).keydown(function(e) { if(e.which==9){obj.val(obj.val()+"\t");return false;} });
